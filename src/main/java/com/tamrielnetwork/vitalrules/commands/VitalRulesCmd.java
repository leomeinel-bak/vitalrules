...
import com.tamrielnetwork.vitalrules.utils.Utils;
...
public class VitalRulesCmd implements TabExecutor {
	...
	private void toggleFly(CommandSender sender, String[] args) {
		...
		//Check args length
		if (args.length == 1) {
			if (!sender.hasPermission("vitalrules.fly")) {
		...
		// Check args length
		if (args.length == 2) {
			if (!sender.hasPermission("vitalrules.fly.others")) {
				Utils.sendMessage(sender, "no-perms");
				return;
			}
	...
	private void toggleFlySpeed(CommandSender sender, String[] args) {
		...
		//Check args length
		if (args.length == 2) {
			if (!sender.hasPermission("vitalrules.flyspeed")) {
				Utils.sendMessage(sender, "no-perms");
				return;
			}
			try {
		...
		// Check args length
		if (args.length == 3) {
			if (!sender.hasPermission("vitalrules.flyspeed.others")) {
				Utils.sendMessage(sender, "no-perms");
				return;
			}
			if (Bukkit.getPlayer(args[1]) == null) {
	...
	public @Nullable List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command, @NotNull String alias, @NotNull String[] args) {
		...
		switch (args.length) {
			...
			case 2, 3 -> {
				if (sender.hasPermission("vitalrules.flyspeed") && args[0].equals("flyspeed")) {
					tabComplete.addAll(List.of(new String[]{"1", "2", "3", "4", "5", "6", "7", "8", "9"}));
				}
				if (sender.hasPermission("vitalrules.fly") && args[0].equals("fly")) {
					tabComplete = null;
				}
			}
